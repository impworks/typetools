	...
	public static class FunctionalHelper
	{
		...
		public static Type[] GetArgumentTypes(this Type type)
		{
			if(!type.IsCallableType())
				throw new ArgumentException(string.Format("Type '{0}' is not callable!", type.Name));
		...
		public static Type GetReturnType(this Type type)
		{
			...
			if(!type.IsFuncType())
				throw new ArgumentException(string.Format("Type '{0}' is not a callable type!", type.Name));
		...
		{
			if(args.Length > 16)
				throw new ArgumentException("Func<> can have up to 16 arguments!");
		...
		{
			if (args.Length > 16)
				throw new ArgumentException("Action<> can have up to 16 arguments!");
		...
		public static Type CreateTupleType(params Type[] args)
		{
			...
			if(args.Length > 8)
				throw new ArgumentException("Tuple<> can have up to 8 type arguments!");
		...
		public static Type DiscardParameters(this Type type, int count)
		{
			if (type.IsActionType())
			{
				...
				if(newCount < 0)
					throw new ArgumentException(string.Format("Cannot discard more than {0} parameters from type '{1}'!", argCount, type));
		...
		public static Type AddParameters(this Type type, params Type[] ps)
		{
			if (type.IsActionType())
			{
				...
				if(newCount > 16)
					throw new ArgumentException("An Action cannot have more than 16 arguments!");
			...
			if (type.IsFuncType())
			{
				...
				if (newCount > 16)
					throw new ArgumentException("A Func cannot have more than 16 arguments!");
